...
def make_ads(output,full_fnames,pic_link):
    os.mkdir(output)
...
def FGO_5th_anniversary_ads():
    ...
    if os.path.exists(output):
        if len(os.listdir(output)) < len(full_fnames):
            print('File will be updated')
            os.remove(output)
            make_ads(output,full_fnames,pic_link)
        else:
            print('5th anniv ads file is already updated')
    else :
        make_ads(output,full_fnames,pic_link)
